...
RUN apt-get install libgdbm-dev libxml-simple-perl perl libperl5.28 libdbi-perl\
                    ...
                    git libconfig-yaml-perl php7.3-mbstring apache2 wget nano -y

RUN cpan -i XML::Entities Archive::Zip Mojolicious Plack::Handler Net::IP
...
#RUN sed -i 's/PerlSetVar OCS_DB_PWD ocs/PerlSetVar OCS_DB_PWD root/g' /etc/apache2/sites-available/z-ocsinventory-server.conf
